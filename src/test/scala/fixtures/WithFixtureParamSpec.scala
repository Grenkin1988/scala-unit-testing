...
class WithFixtureParamSpec extends FixtureAnyFlatSpec with Matchers {
  ...
  override protected def withFixture(test: OneArgTest): Outcome = {
    currencies = (1 to random.between(100, 201))
      .map(_ => {
        val randomAmount = random.between(10, 101)
        val randomCurrencyCode =
          currencyCodes(random.between(0, currencyCodes.length))
  ...
  it should "have totalCost of Dollars more than $10" in {
    f => // "f" for Fixture (WithFixtureParamSpec class itself)
