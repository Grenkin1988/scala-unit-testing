...
trait ProductsDb {
  ...
  def saveDepositProduct(product: Deposits): Unit =
    depositProducts += (product.id -> product)
  def saveLendingProduct(product: Lending): Unit =
...
trait AccountsDb {
  ...
  def saveDepositsAccount(account: DepositsAccount): Unit =
    depositAccounts += (account.id -> account)
  def saveLendingAccount(account: LendingAccount): Unit =
    lendingAccounts += (account.id -> account)
  def getDepositAccount(id: UUID): Option[DepositsAccount] =
    depositAccounts.get(id)
  def getLendingAccount(id: UUID): Option[LendingAccount] =
    lendingAccounts.get(id)
