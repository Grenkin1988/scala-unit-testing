...
trait ProductService extends ProductsDb {
  ...
  /** // todo: (challenge?) how to disallow products of same name?
    ...
    */
  def addNewDepositProduct(
      name: String,
      minBalance: Int,
      ratePerYear: Double,
      transactionsAllowedPerMonth: Int = 2
  ): UUID = {
    val product = name match {
      case "CoreChecking" => new CoreChecking(Dollars(minBalance), ratePerYear)
      case "StudentCheckings" =>
        new StudentCheckings(Dollars(minBalance), ratePerYear)
      case "RewardsSavings" =>
        new RewardsSavings(
          Dollars(minBalance),
          ratePerYear,
          transactionsAllowedPerMonth
        )
    }
  ...
  def addNewLendingProduct(
      annualFee: Double,
      apr: Double,
      rewardsPercent: Double
  ): UUID = {
