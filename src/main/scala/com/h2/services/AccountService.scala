...
trait AccountService
  ...
  def openDepositAccount(
      customerId: UUID,
      productId: UUID,
      amount: Dollars
  ): UUID = {
    require(
      getCustomer(customerId).nonEmpty,
      s"no customer found with id=$customerId"
    )

    val maybeProduct = getDepositProduct(productId)
    require(
      maybeProduct.nonEmpty,
      s"no deposits product found with id=$productId"
    )

    val account =
      new DepositsAccount(getCustomer(customerId).get, maybeProduct.get, amount)
    saveDepositsAccount(account)
    account.id
  }

  def openLendingAccount(
      customerId: UUID,
      productId: UUID,
      balance: Dollars = Dollars(0)
  ): UUID = {
    require(
      getCustomer(customerId).nonEmpty,
      s"no customer found with id=$customerId"
    ...
    require(
      maybeProduct.nonEmpty,
      s"no lending product found with id=$productId"
    )

    val account =
      new LendingAccount(getCustomer(customerId).get, maybeProduct.get, balance)
  ...
  def deposit(accountId: UUID, dollars: Dollars): Unit = {
    ...
    require(
      maybeAccount.nonEmpty,
      "A valid deposits account Id must be provided"
    )
  ...
  def requestCurrency(accountId: UUID, currency: Currency): Unit = {
    ...
    println(
      s"The ${currency.amount} ${currency.code} will be posted to your nearest branch in 2 days."
    )
  }
  ...
  def useCreditCard(accountId: UUID, dollars: Dollars): Unit = {
    ...
    require(
      maybeAccount.nonEmpty,
      "A valid lending account Id must be provided"
    )
    maybeAccount.get withdraw dollars
  }
